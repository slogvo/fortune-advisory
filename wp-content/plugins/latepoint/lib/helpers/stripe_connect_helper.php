...
class OsStripeConnectHelper {
	 ...
	 * @return void
	 */
	public static function output_refund_button_for_transaction(OsTransactionModel $transaction) {
        if($transaction->is_new_record()) return;
        if($transaction->processor != self::$processor_code) return;
        if($transaction->is_fully_refunded()) return;
        ...
        echo '<div class="transaction-refund-settings">';
            ...
            echo '<div class="refund-settings-fields">';
                ...
                echo OsFormHelper::select_field( 'transaction_refund[portion]', false, ['full' => $full_amount_label, 'custom' => __('Custom', 'latepoint')], 'full', [ 'class' => 'size-small refund-portion-selector', 'theme' => 'simple' ] );
                echo '<div class="custom-charge-amount-wrapper" style="display: none;">';
                echo OsFormHelper::money_field( 'transaction_refund[custom_amount]', false, ($transaction->amount - $transaction->get_total_refunded_amount()), [ 'class' => 'size-small', 'theme' => 'simple' ] );
    ...
    public static function refund_transaction(OsTransactionModel $transaction, $custom_amount = null) : OsTransactionRefundModel {
        if($transaction->is_new_record() || $transaction->processor != self::$processor_code) throw new Exception('Invalid Transaction');
        $refund_data = [
